...
const COLORS = ['#3b82f6', '#f59e0b', '#ef4444', '#8b5cf6', '#14b8a6', '#ec4899', '#10b981']
...
export default function Analytics() {
  ...
  const totalSpent = filtered.reduce((sum, r) => sum + (r.total_amount ?? 0), 0)
  ...
  const monthlySpending = Object.entries(byMonth).map(([month, amount]) => ({
    month, amount: Math.round(amount * 100) / 100
  }))
    ...
    <div>
      ...
      <div className="grid grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
          ...
          <div className="text-2xl font-bold text-gray-800">{topStores[0]?.store ?? '—'}</div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
          ...
          <div className="text-2xl font-bold text-gray-800">{mostVisited?.store ?? '—'}</div>
          <p className="text-xs text-gray-500 mt-1">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100">
        <div className="p-6 border-b border-gray-100">
          <h2 className="text-lg font-semibold text-gray-800">Top stores by spending</h2>
        </div>
        {topStores.length === 0 ? (
              ...
              <tbody className="divide-y divide-gray-100">
                {topStores.map((store, i) => (
                  <tr key={i} className="hover:bg-gray-50">
                    <td className="px-6 py-4 text-sm font-medium text-gray-800">{store.store}</td>
                    <td className="px-6 py-4 text-sm text-gray-600">{store.receipts}</td>
                    <td className="px-6 py-4 text-sm font-medium text-gray-800">{store.total.toFixed(2)} €</td>
                  </tr>
                ))}
              </tbody>
            </table>
