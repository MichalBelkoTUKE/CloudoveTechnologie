...
export default function ReceiptsHistory() {
  ...
  useEffect(() => {
    ...
    if (dateFrom) {
      result = result.filter(r => r.receipt_date && r.receipt_date >= dateFrom)
    }
    if (dateTo) {
    ...
    if (minAmount) {
      result = result.filter(r => (r.total_amount ?? 0) >= parseFloat(minAmount))
    }
    if (maxAmount) {
      result = result.filter(r => (r.total_amount ?? 0) <= parseFloat(maxAmount))
    }
