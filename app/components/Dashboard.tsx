...
export default function Dashboard() {
  ...
  receipts.forEach(r => {
    ...
    const month = new Date(r.receipt_date).toLocaleString('en', { month: 'short' })
    spendingByMonth[month] = (spendingByMonth[month] ?? 0) + (r.total_amount ?? 0)
  })
  const spendingOverTime = Object.entries(spendingByMonth).map(([month, amount]) => ({ month, amount: Math.round(amount) }))
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-4 gap-6 mb-8">
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
          <div className="flex items-center justify-between mb-2">
            <span className="text-gray-600 text-sm">Total spent this month</span>
            <Euro className="w-5 h-5 text-blue-500" />
          </div>
          <div className="text-3xl font-bold text-gray-800">{totalSpent.toFixed(2)} €</div>
        </div>
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
          <div className="flex items-center justify-between mb-2">
          ...
          <div className="text-3xl font-bold text-gray-800">{receipts.length}</div>
        </div>
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
          <div className="flex items-center justify-between mb-2">
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
          ...
          <div className="text-3xl font-bold text-gray-800">{avgValue.toFixed(2)} €</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6 mb-8">
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
          ...
          {receipts.length > 0 ? (
            <ResponsiveContainer width="100%" height={280}>
              <PieChart>
                <Pie data={categoryData} cx="50%" cy="50%" outerRadius={100} dataKey="value" label={(e) => `${e.name}: ${e.value}€`}>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">Spending over time</h2>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100">
        <div className="p-6 border-b border-gray-100 flex items-center justify-between">
          ...
          <Link
            to="/dashboard/upload"
            className="px-4 py-2 bg-gradient-to-r from-blue-600 to-violet-600 text-white rounded-lg hover:shadow-lg transition-all text-sm"
          >
            Upload new receipt
          </Link>
        </div>
        <div className="overflow-x-auto">
          {recentReceipts.length === 0 ? (
            <div className="p-12 text-center text-gray-400">
              No receipts yet —{' '}
              <Link to="/dashboard/upload" className="text-blue-600 hover:underline">
              ...
              <tbody className="divide-y divide-gray-100">
                {recentReceipts.map((receipt) => (
                  <tr key={receipt.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 text-sm text-gray-800">{receipt.receipt_date ?? '—'}</td>
                    <td className="px-6 py-4 text-sm text-gray-800">{receipt.merchant_name ?? 'Unknown'}</td>
