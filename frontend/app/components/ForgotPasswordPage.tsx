...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await requestPasswordReset(email, "http://localhost:5173/reset-password");
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Nepodarilo sa odoslať email",
      );
      setLoading(false);
      return;
    }
          ...
          {sent ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full px-6 py-3 bg-linear-to-r from-blue-600 to-violet-600 text-white rounded-lg hover:shadow-lg transition-all disabled:opacity-50"
              >
